...
| 4-6 | 引き継ぎコード実装 — **実装完了 (2026-06-12)**: Functions に callable 2本を追加（`issue_transfer_code`: SHA-256ハッシュをIDとして `transfer_codes` に保存・15分有効・1回限り / `redeem_transfer_code`: トランザクションで検証+使用済み化しカスタムトークン発行）。クライアントは設定画面の発行ダイアログ（コピー付き）+ ログイン画面の「引き継ぎコードでログイン」。**要デプロイ + 初回のみ IAM 設定**（Functions 実行サービスアカウントに `roles/iam.serviceAccountTokenCreator` 付与。カスタムトークン署名に必要） | D-4 |
