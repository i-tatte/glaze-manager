...
| 1-4 | `findOrCreate` 系をマップ事前構築 + バッチ作成に書き換え、インポートの N+1 を解消 — **完了 (2026-06-12)**: インポートを2パス化（全行パース → 顔料一括作成 → ID 解決1回）、タグ保存も `addTags` バッチ化。同名原料の競合（複数端末が同時に findOrCreate する場合のみ発生）は、完全防止に名前インデックスコレクション等の追加設計を要する一方、単一ユーザー・端末1〜2台の利用形態では発生確率が実質ゼロのため**受容する**（事前の名前重複チェックは実装済み） | H-3 |
...
| 2-2 | 画像パスに docId を含める形式へ移行し、Functions をパスから docId 直接解決に変更（クエリ排除）。旧パス形式との互換処理を入れる — **実装完了 (2026-06-12)**: 新形式 `users/{uid}/test_pieces/images/{docId}/{uuid}.jpg`。新規作成は ID 先行発行 (`createTestPieceId` + `setTestPiece`)。Functions は docId 直接取得 + 旧形式は imagePath クエリでフォールバック。**Functions の再デプロイ含め完了** | C-1 |
...
| # | 作業 | 対応する指摘 |
|---|---|---|
| 4-1 | 削除時の参照チェックと警告 — **完了 (2026-06-12)**: 全削除ダイアログ（原料×2箇所・釉薬・素地土・焼成雰囲気・焼成プロファイル）に「使用中◯件」と削除後の影響（「不明な原料」表示・詳細閲覧不可・「未設定」化）の警告を追加。集計はデータプロバイダの `.future` 読みで追加クエリなし。論理削除（archivedフラグ）は警告運用で問題が出た場合の将来課題とする | H-2 |
| 4-2 | Excel インポートのヘッダー検証 + 取り込みプレビュー — **完了 (2026-06-12)**: パース（書き込みなし）と確定を分離し、件数・スキップ・新規原料/顔料を確認ダイアログで提示してから書き込む。ヘッダー形式（顔料・備考列の位置）を検証し不一致は日本語エラー。パーサは静的メソッド化して単体テスト4件を追加。テストで「顔料と備考の間の任意列が原料として誤登録される」潜在バグを検出し修正 | M-4 |
| 4-3 | マイナス検索のチップ表現 — **完了 (2026-06-12)**: 「-語」チップを除外アイコン+「◯◯ を除外」表示に。なお語分割の全角スペース問題は、Dart の `\s` が U+3000 を含むため実際には存在しなかった（M-5 の指摘を訂正） | M-5 |
| 4-4 | 編集フォームのバリデーション強化 — **完了 (2026-06-12)**: レシピ行に「原料選択済みなら正の数値必須」「配合量入力済みなら原料選択必須」のバリデータを追加。保存時の `?? 0.0` での黙殺を廃止 | M-6 |
| 4-5 | 匿名 → 永続アカウントのリンク実装 — **完了 (2026-06-12)**: `AuthService.linkWithGoogle` / `linkWithEmailAndPassword` を追加し、設定画面の「アカウント」セクションから実行可能（匿名時のみ表示）。連携済みエラー等は日本語メッセージで通知。Google 認証情報の取得処理はサインインとリンクで共通化し、Windows での失敗の握りつぶしも解消 | M-7, D-4 |
| 4-6 | 引き継ぎコード実装 — **実装完了 (2026-06-12)**: Functions に callable 2本を追加。仕様は「**有効期限なし**（メモ保管できる代替ログイン手段）・1回使用で無効・再発行で旧コード無効（有効コードは常にユーザーごと最大1つ）」。コードは平文保存せず SHA-256 ハッシュをドキュメントIDとして `transfer_codes` に保存。引き換えはトークン発行成功後に使用済み化（発行失敗でコードが消費されない順序）+ トランザクションで二重使用防止。クライアントは設定画面の発行ダイアログ（コピー付き）+ ログイン画面の「引き継ぎコードでログイン」。**IAM 必須**: 実行サービスアカウント `942515568123-compute@developer.gserviceaccount.com` に `roles/iam.serviceAccountTokenCreator`（カスタムトークン署名に必要。REST 経由の E2E テストで未付与だと PERMISSION_DENIED になることを確認済み） | D-4 |
